suite('irrFilter', () => {
    benchmark(
        ...
        () => {
            ...
            for (let i = 1; i < this.values.length; i += 1) {
                results[i] =
                    this.values[i] * this.feedforward[0] * this.feedback[0] -
                    this.values[i - 1] +
                    results[i - 1] * this.feedforward[1] * this.feedback[1];
            }
        },
        {
            setup: () => {
                ...
                this.feedback = new Float32Array([Math.random() - Math.round(Math.random()), Math.random() - Math.round(Math.random())]);
                this.feedforward = new Float32Array([Math.random() - Math.round(Math.random()), Math.random() - Math.round(Math.random())]);
    ...
    benchmark(
        ...
        (deferred) => {
            ...
            valuesBufferSource.connect(iirFilter).connect(offlineAudioContext.destination);
