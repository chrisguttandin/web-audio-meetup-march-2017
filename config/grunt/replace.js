        ...
        'manifest': {
            ...
            options: {
                patterns: [ {
                    ...
                    match: /"\/web-audio-meetup-march-2017(?<filename>\/scripts\/main(?:-es(?:2015|5))?.[a-z0-9]+.js)":\s"[a-z0-9]+"/g,
                    ...
                    match: /"\/web-audio-meetup-march-2017(?<filename>\/styles\/styles\.[a-z0-9]*\.css)":\s"[a-z0-9]+"/g,
                    replacement: (_, filename) => {
                        return `"/web-audio-meetup-march-2017${ filename }": "${ computeHashOfFile(`build/web-audio-meetup-march-2017${ filename }`, 'sha1', 'hex') }"`;
                    }
                }, {
                    // Replace the hash value inside of the hashTable for "/(index|start).html" because it was modified before.
                    match: /"\/web-audio-meetup-march-2017\/(?<filename>index|start)\.html":\s"[a-z0-9]+"/g,
                    replacement: (_, filename) => {
